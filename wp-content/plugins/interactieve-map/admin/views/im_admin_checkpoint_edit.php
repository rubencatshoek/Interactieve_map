...
// Include model:
include MY_CATERING_PLUGIN_MODEL_DIR . "/CateringCategory.php";

// Declare class variable:
$catering_categories = new CateringCategory();
...
$post_array = $catering_categories->getPostValues();

// Get the GET data in filtered array
$get_array = $catering_categories->getGetValues();
...
if (!empty($get_array)) {
    ...
    if (isset($get_array['action'])) {
        $action = $catering_categories->handleGetAction($get_array);
    }
}
/*
echo '<pre>';
...
if (!empty($post_array)) {
    ...
    if (isset($post_array['add'])) {
        ...
        $result = $catering_categories->save($post_array);
        if ($result) {
            // Save was succesfull
            $add = true;
        } else {
...
// Save event categorie
        $catering_categories->update($post_array);
    }
}
